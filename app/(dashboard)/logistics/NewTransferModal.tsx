...
const CARRIERS = ['DHL', 'FedEx', 'USPS', 'UPS', 'Other']

export default function NewTransferModal({ skus, onClose }: Props) {
  ...
  const sampleSkus = skus.filter(s => s.sku_type === 'Sample')
  const productSkus = skus.filter(s => s.sku_type !== 'Sample')

  return (
    <Dialog open onOpenChange={() => onClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>New Transfer — JP → US Warehouse</DialogTitle>
        </DialogHeader>
